...
func NewActionApi(
	...
	api.GetActionInfoByActionID(apiRoutes)
...
//	@Failure		401	{object}	schemas.ErrorResponse
//	@Failure		500	{object}	schemas.ErrorResponse
//	@Router			/action/info/action/:idAction [get]
func (api *ActionApi) GetActionInfoByActionID(apiRoutes *gin.RouterGroup) {
	apiRoutes.GET("/action/:idAction", func(ctx *gin.Context) {
		...
		response, err := api.controller.GetActionByActionID(idInt)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, &schemas.ErrorResponse{
				Error: err.Error(),
			})
