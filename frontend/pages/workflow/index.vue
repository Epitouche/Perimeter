...
const loadWorkflowState = () => {
  if (isClient) {
    actionIsSelected.value = JSON.parse(localStorage.getItem(ACTION_SELECTED_KEY) || 'false');
    reactionIsSelected.value = JSON.parse(localStorage.getItem(REACTION_SELECTED_KEY) || 'false');
    showNavBar.value = JSON.parse(localStorage.getItem(SHOW_NAVBAR_KEY) || 'true');
    showCancelButton.value = JSON.parse(localStorage.getItem(SHOW_CANCEL_BUTTON_KEY) || 'false');
    showCreateButton.value = JSON.parse(localStorage.getItem(SHOW_CREATE_BUTTON_KEY) || 'false');
    reactionButtonisDisabled.value = JSON.parse(localStorage.getItem(REACTION_BUTTON_DISABLED_KEY) || 'true');

    const queryActionId = Array.isArray(route.query.actionId) ? route.query.actionId[0] : route.query.actionId;
    ...
    const queryReactionId = Array.isArray(route.query.reactionId) ? route.query.reactionId[0] : route.query.reactionId;
...
const saveWorkflowState = () => {
  if (isClient) {
    localStorage.setItem(ACTION_SELECTED_KEY, JSON.stringify(actionIsSelected.value));
    localStorage.setItem(REACTION_SELECTED_KEY, JSON.stringify(reactionIsSelected.value));
    localStorage.setItem(SHOW_NAVBAR_KEY, JSON.stringify(showNavBar.value));
    localStorage.setItem(SHOW_CANCEL_BUTTON_KEY, JSON.stringify(showCancelButton.value));
    localStorage.setItem(SHOW_CREATE_BUTTON_KEY, JSON.stringify(showCreateButton.value));
    localStorage.setItem(REACTION_BUTTON_DISABLED_KEY, JSON.stringify(reactionButtonisDisabled.value));
    ...
    if (reactionOptions.value) {
      localStorage.setItem(REACTION_OPTIONS_KEY, JSON.stringify(reactionOptions.value));
    }
  }
};
