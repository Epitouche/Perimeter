...
<template>
  <UContainer :ui="{ padding: '!px-0', constrained: 'max-w-full max-h-full' }"
    ...
    <div v-for="(app, index) in apps" :key="index">
      <UContainer v-if="styling === 'card'" :ui="{ padding: '!px-0', constrained: 'max-w-none' }"
        ...
        <img v-if="getServiceDetails(app.name)?.icon" :src="getServiceDetails(app.name)?.icon" alt="" class="icon_circle">
        <UButton 
        :class="[
      ...
      v-if="styling === 'button'" :ui="{ padding: '!px-0', constrained: 'max-w-none' }"
        class="custom_card_circle" tabindex="0" :style="{ backgroundColor: getServiceDetails(app.name)?.color || '#ccc' }"
        @mouseenter="hover[app.name] = true" @mouseleave="hover[app.name] = false" @click="onClick(app.name)">
        <img v-if="getServiceDetails(app.name)?.icon && !hover[app.name]" :src="getServiceDetails(app.name)?.icon" alt="" class="icon_circle">
