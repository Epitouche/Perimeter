  ...
  <UContainer :key="componentKey" :ui="{ padding: '!px-0', constrained: 'max-w-full max-h-full' }"
    ...
    <div v-for="area in areas" :key="area.id">
      <UContainer :ui="{ padding: '!px-0', constrained: 'max-w-none' }"
        ...
        <h5 class="clamp-2-lines capitalize text-center break-words w-full">
      ...
      <UModal ref="focusDiv" v-model="areaIsOpen[area.id]" tabindex="0" :ui="{
        ...
        <div
          class="flex flex-col gap-12 font-semibold text-white rounded-custom_border_radius pl-20 pr-16 py-10 w-full max-h-[90vh]"
          ...
          <div class="scrollbar-hidden w-full overflow-x-scroll min-h-[10vh]">
            <h4 class="self-start b-2 whitespace-nowrap">Description:</h4>
            <h5 class="pl-10 whitespace-nowrap">{{ area.description }}</h5>
          </div>

          <UContainer :ui="{ padding: '!px-0', constrained: 'max-w-none' }"
            class="scrollable-element w-full bg-custom_color-bg_section overflow-y-scroll min-h-[10vh] rounded-lg text-black">
            <h4 v-if="!selectedAreaResult" class="px-1">
              No Result
            </h4>
          ...
          <div class="flex flex-row justify-end items-center gap-5">
            ...
            <USlideover v-model="editAreaIsOpen[area.id]">
              <UForm :state="state[area.id]"
                ...
                <UFormGroup v-for="(value, key) in filteredState(area.id)" :key="key" :label="formatName(key)"
                  :name="key" :ui="{ label: { base: 'capitalize text-xl pl-3' } }">
