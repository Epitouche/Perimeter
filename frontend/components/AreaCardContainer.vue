  ...
  <UContainer :ui="{ padding: '!px-0', constrained: 'max-w-full max-h-full' }"
    ...
    <div v-for="area in areas" :key="area.id">
      <UContainer :ui="{ padding: 'px-0', constrained: 'max-w-none' }"
        class="
        flex flex-col justify-center items-center text-white font-extrabold text-6xl rounded-custom_border_radius w-[5em] h-[4.5em]"
        :style="{ backgroundColor: area.action.service.color }" @click="toggleAreaModal(area.id)">
        <h2 class="clamp-2-lines capitalize text-4xl text-center break-words w-full pb-6">
          {{ formatName(area.action.name) }}
        </h2>
        <img :src="area.action.service.icon" :alt="area.action.service.name" class="w-16 h-16 p-0 self-start">
        <img :src="area.reaction.service.icon" :alt="area.reaction.service.name" class="w-16 h-16 p-0 self-end">
      </UContainer>
      <UModal v-model="areaIsOpen[area.id]" :ui="{
        ...
        <div class="flex flex-col gap-14 font-semibold text-white rounded-custom_border_radius pl-20 pr-12 py-10 w-full"
          ...
          <p class="self-start text-5xl"><b>Description</b>: Desc will go here</p>
      ...
      <UModal v-model="confirmDeletionIsOpen[area.id]" :ui="{
        ...
        <div class="flex flex-row justify-end items-center gap-5 pt-5">
          <UButton class="bg-opacity-0 border-custom_border_width text-2xl font-semibold py-3 px-5"
            :style="{ borderColor: area.action.service.color, color: area.action.service.color }"
