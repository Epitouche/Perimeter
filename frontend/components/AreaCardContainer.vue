    ...
    <div v-for="area in areas" :key="area.id">
      ...
      <UModal
       v-model="areaIsOpen[area.id]"
        :ui="{ background: `bg-custom_color-${area.action.service.name}`, width: 'w-2/5', base: 'relative flex flex-col gap-14 text-5xl font-semibold text-white pl-20 pr-12 py-10' }">
        ...
        <div class="capitalize self-start flex flex-row items-center gap-5">
          ...
          <p>{{ area.action.service.name }}: {{ formatName(area.action.name) }}</p>
        </div>
        <div class="capitalize self-start flex flex-row items-center gap-5">
          <UIcon name="i-bytesize-ban" class="w-16 h-16 p-0" />
          <p>{{ area.reaction.service.name }}: {{ formatName(area.reaction.name) }}</p>
        </div>
      ...
      <UModal
       v-model="confirmDeletionIsOpen[area.id]" :ui="{
        ...
        <h2 class="text-4xl font-semibold">Are you sure you want to delete this area?</h2>
        <p class="text-2xl">This action cannot be undone!</p>
        <div class="flex flex-row justify-end items-center gap-5 pt-5">
          <UButton :class="[`text-custom_color-${area.action.service.name} !border-custom_color-${area.action.service.name}`, 'bg-opacity-0 border-custom_border_width text-2xl font-semibold py-3 px-5']" @click="cancelDeletion(area.id)">Cancel</UButton>
          <UButton :class="[`bg-custom_color-${area.action.service.name}`, 'text-white text-2xl font-semibold py-3 px-5']" @click="onDelete(area.id)">Delete</UButton>
        </div>
      </UModal>
    </div>
  </UContainer>
