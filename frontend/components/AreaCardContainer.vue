...
const onDelete = async (area: Area) => {
  console.log("area to delete:", area); ///////
  if (confirmDeletionIsOpen[area.id]) {
    try {
    ...
    toggleConfirmDeletionModal(area.id);
    return;
  }
  toggleConfirmDeletionModal(area.id);
  toggleAreaModal(area.id);
};
...
class="text-white text-2xl font-semibold py-3 px-5"
            :style="{ backgroundColor: area.action.service.color }" @click="onDelete(area)">Delete</UButton>
        </div>
      </UModal>
    </div>
  </UContainer>
