      ...
      >
        <UContainer
          ...
          :class="[
            ...
            'basis-1/4 flex flex-col justify-end items-center flex-wrap gap-10 text-white font-extrabold text-6xl p-8 rounded-custom_border_radius w-[5em] h-[4.5em]',
          ]"
        >
          <UIcon :name="`my-icons:white-${service.name}`" />
          <h2 class="capitalize">{{ service.name }}</h2>
        </UContainer>
      </NuxtLink>
    </div>
  </UContainer>
