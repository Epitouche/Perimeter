...
:state="state[type.id]"
            ...
            <h6 class="text-center self-center -mt-6 max-w-[80%] max-lg:max-w-[85%] max-md:max-w-[90%] max-sm:max-w-[95%]">
              {{ type.description }}
            </h6>
...
v-model="state[type.id][key]" :type="fieldTypes[type.id][key] === 'number' ? 'number' : 'text'
            ...
            <div class="flex flex-row justify-evenly gap-4 pt-4">
              <UButton
                ...
                tabindex="0" @click="toggleConfig(type.id)">
                <h6>Cancel</h6>
              </UButton>
              <UButton type="submit" class="font-semibold px-6 py-5 max-lg:py-3 max-md:py-2 max-sm:py-1" tabindex="0">
                <h6>Submit</h6>
              </UButton>
            </div>
          </UForm>
