...
<template>
    <UContainer :ui="{ padding: 'px-0', constrained: 'max-w-full' }"
        ...
        <div v-for="type in types" :key="type.id">
            <UContainer :ui="{ padding: 'px-0', constrained: 'max-w-none' }" :class="[
                ...
                <h2 class="clamp-2-lines capitalize text-4xl text-center break-words w-full">
                    {{ formatString(type.name) }}
                </h2>
                <p class="text-2xl">{{ type.description }}</p>
            </UContainer>

            <UModal v-model="configIsOpen[type.id]"
                :ui="{ base: `relative text-left rtl:text-right flex flex-col p-10 border-custom_border_width border-custom_color-${serviceInfo.name}` }">
                <template #default>
                    <UForm :state="state[type.id]" class="flex flex-col gap-12 p-5 bg-custom_color-bg_section"
                        @submit.prevent="onSubmit(type.id)">
                        <h2 class="text-center text-6xl font-semibold pb-2">{{ formatString(type.name) }}</h2>
                        <UFormGroup v-for="(value, key) in state[type.id]" :key="key" :label="key" :name="key"
                            :ui="{ label: { base: 'capitalize text-2xl' } }">
                            <UInput v-model="state[type.id][key]" :ui="{
