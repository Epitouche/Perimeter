export default defineEventHandler(async (event) => {
  try {
    ...
    if (
      typeof params.actionOptions === "object" &&
      params.actionOptions !== null
    ) {
      const newTimeOptions = {
        hour: params.actionOptions.hour ? Number(params.actionOptions.hour) : 0,
        minute: params.actionOptions.minute
          ? Number(params.actionOptions.minute)
          : 0,
      };
    ...
    if (
      typeof params.reactionOptions === "object" &&
      params.reactionOptions !== null
    ) {
      reactionString =
        `{ ` +
        Object.entries(params.reactionOptions)
          .map(([key, value]) => {
